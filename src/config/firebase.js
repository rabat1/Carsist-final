...
async function uploadDocument(response) {
  ...
  const url = await storage()
    .ref('user-documents/' + response['name'])
    .getDownloadURL();
  return url;
}
...
async function updateMechanicProfile(params) {
  ...
  await firestore().collection('mechanic').doc(user.id).update({
    name,
    contactno,
    services,
  });
...
async function addride(params) {
  ...
  const docRef = await firestore().collection('rides').add({
    mechanicId,
    userId: id,
    pickup,
    username: name,
...
async function updateRide(id, status) {
  await firestore().collection('rides').doc(id).update({
    status,
  });
}
...
async function addBill(
  ...
  // console.log(id,status,cost,date,odometer,service_name,mechanic_name,userid,mechanicid);
  let payment = 'not done';
  await firestore().collection('rides').doc(id).update({
    status,
    cost,
    date,
...
async function addExpense(form, userid) {
  const {cost, date, odometer, service_name, mechanic_name, mechanicId} = form;
